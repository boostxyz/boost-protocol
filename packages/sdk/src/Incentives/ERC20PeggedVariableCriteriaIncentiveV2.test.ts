...
let fixtures: Fixtures,
  erc20: MockERC20,
  erc721: MockERC721,
  erc20PeggedVariableCriteriaIncentiveV2: ERC20PeggedVariableCriteriaIncentiveV2,
  budgets: BudgetFixtures,
  boost: Boost;

describe("ERC20PeggedVariableCriteriaIncentiveV2", () => {
  beforeAll(async () => {
  ...
  beforeEach(async () => {
    ...
    erc20PeggedVariableCriteriaIncentiveV2 = fixtures.core.ERC20PeggedVariableCriteriaIncentiveV2({
  ...
  describe("Remaining Claims", () => {
    test("should calculate remaining claim potential correctly", async () => {
      const remaining = await erc20PeggedVariableCriteriaIncentiveV2.getRemainingClaimPotential();
      const limit = await erc20PeggedVariableCriteriaIncentiveV2.limit();
      const totalClaimed = await erc20PeggedVariableCriteriaIncentiveV2.totalClaimed();
      expect(remaining).toBe(limit - totalClaimed);
    });

    test("should return true for canBeClaimed when claims remain", async () => {
      const canBeClaimed = await erc20PeggedVariableCriteriaIncentiveV2.canBeClaimed();
  ...
  describe("Claim Data", () => {
    test("should properly encode claim data", () => {
      ...
      const encodedData = erc20PeggedVariableCriteriaIncentiveV2.buildClaimData(rewardAmount);
  ...
  describe("Owner", () => {
    test("should return correct owner", async () => {
      const owner = await erc20PeggedVariableCriteriaIncentiveV2.owner();
  ...
  describe("Current Reward", () => {
    test("should return valid current reward", async () => {
      const currentReward = await erc20PeggedVariableCriteriaIncentiveV2.currentReward();
  ...
  describe("getIncentiveScalar", () => {
    test("should return a valid scalar for function-based criteria", async () => {
      ...
      const scalar = await erc20PeggedVariableCriteriaIncentiveV2.getIncentiveScalar({
        hash,
        chainId: 31337,
    ...
    test("should throw NoMatchingLogsError for event criteria with no matching logs", async () => {
      ...
      try {
        await erc20PeggedVariableCriteriaIncentiveV2.getIncentiveScalar({
          hash,
          chainId: 31337,
          knownSignatures: allKnownSignatures,
        });
  ...
  test("can properly encode a uint256", () => {
    //@ts-ignore
    const incentive = fixtures.core.ERC20PeggedVariableCriteriaIncentiveV2();
  ...
  test("can clawback via a budget", async () => {
    ...
    const [amount, address] = await budgets.budget.clawbackFromTarget(
      ...
      erc20PeggedVariableCriteriaIncentiveV2.buildClawbackData(1n),
      boost.id,
      0,
    );
  ...
  test('can claim', async () => {
    ...
    const incentiveData = erc20PeggedVariableCriteriaIncentiveV2.buildClaimData(signedAmount)
    ...
    const incentiveClaimAmount = await erc20PeggedVariableCriteriaIncentiveV2.decodeClaimData(claimDataPayload)
