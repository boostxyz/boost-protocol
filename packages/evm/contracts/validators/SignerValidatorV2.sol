...
/// @title Signer Validator V2
/// @notice Enhanced implementation that includes referrer verification in signed claims
/// @dev This version requires the referrer to be included in the signed claim data
contract SignerValidatorV2 is ASignerValidatorV2, Ownable, EIP712 {
    using SignatureCheckerLib for address;
    using IncentiveBits for IncentiveBits.IncentiveMap;

    event SignerValidatorV2Initialized(address validatorCaller);
    ...
    bytes32 internal constant _SIGNER_VALIDATOR_TYPEHASH = keccak256(
        "SignerValidatorData(uint256 boostId,uint8 incentiveQuantity,address claimant,bytes incentiveData,address referrer)"
    );

    /// @notice Construct a new SignerValidatorV2
    ...
    function initialize(bytes calldata data_) public virtual override initializer {
        ...
        emit SignerValidatorV2Initialized(validatorCaller_);
    }
        ...
        returns (bool)
    {
        ...
        (BoostClaimDataWithReferrer memory claim) = abi.decode(claimData, (BoostClaimDataWithReferrer));
        (SignerValidatorInputParams memory validatorData) =
            abi.decode(claim.validatorData, (SignerValidatorInputParams));

        bytes32 hash =
            hashSignerData(boostId, validatorData.incentiveQuantity, claimant, claim.incentiveData, claim.referrer);
    ...
    function hashSignerData(
        ...
        return _hashTypedData(
            keccak256(
                abi.encode(
                    _SIGNER_VALIDATOR_TYPEHASH, boostId, incentiveQuantity, claimant, keccak256(incentiveData), referrer
                )
            )
        );
    }
