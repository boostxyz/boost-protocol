...
contract EndToEndSignerValidatorV2 is Test, OwnableRoles {
    ...
    function setUp() public {
        ...
        registry.register(
            ABoostRegistry.RegistryType.VALIDATOR, "LimitedSignerValidatorV2", address(new LimitedSignerValidatorV2())
        );
                            ...
                            instance: address(
                                registry.getBaseImplementation(
                                    registry.getIdentifier(
                                        ABoostRegistry.RegistryType.VALIDATOR, "LimitedSignerValidatorV2"
                                    )
                                )
                            ),
    ...
    function _createClaimDataWithReferrer(
        ...
        bytes32 msgHash =
            LimitedSignerValidatorV2(address(boost.validator)).hashSignerData(0, 1, claimer, claimBytes, referrer);

        bytes memory signature = _signHash(msgHash, validatorSignerKey);

        ALimitedSignerValidatorV2.SignerValidatorInputParams memory validatorData =
